...
??? question "Show Answer"
    ...
    [Cypher](index.md#cypher) is a declarative query language designed for querying graph databases, particularly [Neo4j](index.md#neo4j). It uses ASCII-art-like syntax to represent patterns in graphs, making it intuitive for expressing graph queries. Option A would be something like AES, option C would be like CNNs, and option D would be like ZIP.
...
??? question "Show Answer"
    ...
    In a [knowledge graph](index.md#knowledge-graph), nodes typically represent entities or concepts (like people, places, organizations, or ideas), while edges represent the relationships between them. Option A describes SQL queries, option C describes analytics data, and option D describes errors.
...
??? question "Show Answer"
    ...
    In [Cypher](index.md#cypher), the pattern `(a)-[:KNOWS]->(b)` represents a directed relationship where entity 'a' knows entity 'b'. The parentheses represent nodes, the square brackets contain the relationship type, and the arrow shows direction. Option A describes math notation, option C describes filesystem paths, and option D describes cryptography.
